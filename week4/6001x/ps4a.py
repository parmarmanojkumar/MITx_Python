...
def getWordScore(word, n):
    ...
    for alphabet in word :
        wordscore += SCRABBLE_LETTER_VALUES[alphabet]
    wordscore = (wordscore * len(word)) + (50 * (n == len(word)))
    return wordscore
...
def isValidWord(word, hand, wordList):
    ...
    if (count != len(word)) or (word not in wordList) :
        valid = False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while (calculateHandlen(hand) != 0):
        ...
        displayHand(hand)
        # Ask user for input
        user_ip = raw_input("Enter word, or a \".\" to indicate that you are finished:")
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                wordscore = getWordScore(user_ip,n)
                total_score += wordscore
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print ("\""+ user_ip + "\" earned " +str(wordscore) + " points. Total: "+str(total_score)+" points " )
                # Update the hand 
